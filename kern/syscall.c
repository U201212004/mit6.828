...
sys_exofork(void)
{
    ...
    parent = curenv;
    if((ret = env_alloc(&child, parent->env_id)) < 0)
        return ret;
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
    ...
    tf->tf_eflags |= FL_IF; //允许用户进程中断
    tf->tf_eflags &= ~FL_IOPL_MASK; //用户进程没有IO权限
    tf->tf_cs |= 3;
    env->env_tf = *tf;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    //check envid exists or the caller permission
    if(envid2env(envid, &env, 1) < 0)
        return -E_BAD_ENV;
    //check va is user address space and is page-align
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    //check srcenvid and dstenvid exist, check permission
    if(envid2env(srcenvid, &srcenv, 1) < 0 || envid2env(dstenvid, &dstenv, 1) < 0)
        return -E_BAD_ENV;
    //check srcva >= UTOP and dstva is not page-aligned
