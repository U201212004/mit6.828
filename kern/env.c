...
load_icode(struct Env *e, uint8_t *binary)
{
    ...
    if(elf_header->e_magic != ELF_MAGIC)
        panic("the binary is not a elf file!\n");

    if(elf_header->e_entry == 0)
        panic("the entry point is invalid!\n");
    ...
    for(; ph < eph; ph++){
        ...
        if(ph->p_filesz > ph->p_memsz)
            panic("the file size is larger than memory size!\n");

        region_alloc(e, (void *)ph->p_va, ph->p_memsz);
        //copy from elf to ph->p_va, use memmove instead of memcopy
